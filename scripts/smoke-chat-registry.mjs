...
assert(
  !plain.some((item) =>
    (item.dependencies ?? []).some((dep) =>
      /react-markdown|remark-gfm|^ai$|@ai-sdk/.test(dep),
    ),
  ),
  "Core registry must not pull Markdown or AI SDK",
);
...
await writeFile(
  ...
  JSON.stringify(
    {
      name: "dethink-chat-consumer-smoke",
      private: true,
      type: "module",
      dependencies: deps,
...
await writeFile(
  ...
  '<html lang="en"><head><title>Chat consumer smoke</title></head><body><div id="root"></div><script type="module" src="/main.tsx"></script></body></html>',
);
await writeFile(
