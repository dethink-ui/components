...
const entry = "carousel";
const react18 = process.argv.includes("--react18");
const destination = await mkdtemp(join(tmpdir(), "dethink-carousel-consumer-"));
...
await writeFile(
  ...
  JSON.stringify(
    {
      name: "dethink-carousel-consumer-smoke",
      private: true,
      type: "module",
      dependencies: deps,
...
await writeFile(
  ...
  '<html lang="en"><head><title>Carousel consumer smoke</title></head><body><div id="root"></div><script type="module" src="/main.tsx"></script></body></html>',
);
await writeFile(
  join(destination, "main.tsx"),
  `import { createRoot } from "react-dom/client";
import { Carousel, CarouselContent, CarouselItem, CarouselNext, CarouselPrevious, CarouselDots } from "./src/components/carousel";
import "./src/styles.css";
createRoot(document.getElementById("root")!).render(<>{(["fan", "arc", "ribbon"] as const).map(staging => <Carousel key={staging} staging={staging} inactiveBlur={1.5} aria-label={staging}><CarouselContent>{[0,1,2].map(i => <CarouselItem key={i}><article className="min-h-64 bg-background p-6">Card {i}</article></CarouselItem>)}</CarouselContent><CarouselPrevious/><CarouselDots/><CarouselNext/></Carousel>)}</>);
`,
);
await writeFile(
