...
const entry = "file-upload";
const react18 = process.argv.includes("--react18");
const destination = await mkdtemp(
  join(tmpdir(), "dethink-file-upload-consumer-"),
);
...
assert.equal("motion" in deps, false, "FileUpload must not require Motion");
...
await writeFile(
  ...
  JSON.stringify(
    {
      name: "dethink-file-upload-consumer-smoke",
      private: true,
      type: "module",
      dependencies: deps,
...
await writeFile(
  ...
  '<html lang="en"><head><title>FileUpload consumer smoke</title></head><body><div id="root"></div><script type="module" src="/main.tsx"></script></body></html>',
);
await writeFile(
...
createRoot(document.getElementById("root")!).render(<FileUpload multiple accept=".pdf,.png" maxFiles={3} maxFileSize={1048576} onUpload={async (_file, {signal, onProgress}) => { if (!signal.aborted) onProgress(100); }} />);
`,
);
await writeFile(
