...
const entry = process.argv.includes("--base")
  ? "card-stack"
  : "card-stack-animated";
const react18 = process.argv.includes("--react18");
const destination = await mkdtemp(
  join(tmpdir(), "dethink-card-stack-consumer-"),
);
const selected = [...closure(entry).values()];
assert(
  ![...closure("card-stack").values()].some((item) =>
    (item.dependencies ?? []).some((dep) => /^motion(?:@|$)/.test(dep)),
  ),
  "Base CardStack must not require Motion",
);
const deps = {
...
await writeFile(
  ...
  JSON.stringify(
    {
      name: "dethink-card-stack-consumer-smoke",
      private: true,
      type: "module",
      dependencies: deps,
...
await writeFile(
  ...
  '<html lang="en"><head><title>Card Stack consumer smoke</title></head><body><div id="root"></div><script type="module" src="/main.tsx"></script></body></html>',
);
await writeFile(
  join(destination, "main.tsx"),
  `import { createRoot } from "react-dom/client";
import { Card } from "./src/components/card";
import { ${entry === "card-stack" ? "CardStack" : "CardStackAnimated"} as Deck } from "./src/components/${entry}";
import "./src/styles.css";
createRoot(document.getElementById("root")!).render(<Deck aria-label="Consumer deck" visibleCount={3} getCardLabel={(index) => String(index + 1)}><Card>First</Card><Card>Second</Card></Deck>);
`,
);
await writeFile(
...
await writeFile(
  join(destination, "server.tsx"),
  `import { renderToString } from "react-dom/server";
import { Card } from "./src/components/card";
import { ${entry === "card-stack" ? "CardStack" : "CardStackAnimated"} as Deck } from "./src/components/${entry}";
export const html = renderToString(<Deck defaultActiveIndex={1}><Card>First</Card><Card>Second</Card></Deck>);`,
);
execFileSync(
  join(destination, "node_modules/.bin/vite"),
  ["build", "--ssr", "server.tsx", "--outDir", "dist-ssr"],
  { cwd: destination, stdio: "inherit" },
);
const { html } = await import(
  `file://${join(destination, "dist-ssr/server.js")}`
);
assert.match(
  html,
  /inert=""/,
  "Inactive cards must be inert in server HTML on both supported React versions",
);
