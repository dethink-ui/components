...
| `PromptInput`      | Controlled `value/onValueChange` or local `defaultValue`; `onSend`, `onStop`, `disabled`, `readOnly`, `submitOnEnter` (true), `maxHeight` (192px). Key standalone inputs by conversation ID. `actions` accepts host controls, including an explicitly activated VoiceInput. |
