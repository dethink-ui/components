...
| DotMatrixBackground     | `dot-matrix-background`     | Dot grid + seeded brightness ripples, or a pointer-following local glow                                                                                  |
...
// DotMatrixBackground and StarfieldBackground:
interactive?: boolean;                    // default true — pointer enhancement (disabled under reduced motion)
// DotMatrixBackground only:
mode?: "pulse" | "follow";                // default "pulse" — looping ripples or a glow that follows the mouse
...
```html
<div data-slot="<slug>" class="relative isolate overflow-hidden">
  <!-- forwardRef root -->
  <div
    data-slot="<slug>-layer"
    aria-hidden="true"
...
Data attributes on root: `data-animate`, `data-density`, `data-intensity`, `data-speed`, `data-tone`, `data-reduced-motion` (present only when true), plus `data-direction` (ScanGrid), `data-interactive` (DotMatrix and Starfield), and `data-mode` (DotMatrix).
...
- **DotMatrix** — one static `radial-gradient` dot-pattern div. In `pulse` mode, three overlay divs at seeded origins carry a brighter radial-masked dot pattern and loop through staggered opacity pulses. In `follow` mode, one radial-masked brighter pattern follows mouse coordinates through Motion values; it is hidden until the pointer enters and disabled for touch, offscreen, and reduced-motion states.
